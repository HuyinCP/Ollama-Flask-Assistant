...
document.addEventListener('DOMContentLoaded', function() {
    // Set default model (phải khớp với <option value="..."> trong index.html)
...
function displayMessage(message) {
    ...
    const hasMeta = message.summary != null || message.sentiment != null || message.action != null;
    const meta = hasMeta ? `
        <div class="message-meta">
            <div><span class="meta-label">Tóm tắt:</span> ${escapeHtml(message.summary || '—')}</div>
            <div><span class="meta-label">Cảm xúc:</span> ${message.sentiment ?? '—'}/100</div>
            <div><span class="meta-label">Hành động:</span> ${escapeHtml(message.action || '—')}</div>
        </div>` : '';
